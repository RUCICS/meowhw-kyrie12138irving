...
char* align_alloc(size_t size) {
    size_t page_size = io_blocksize(); // 获取系统页大小
    ...
    // 2. + page_size - 1: 向上取整的技巧，确保无论当前地址在哪，加上这个值后都能跨越到下一个或当前的页边界
    // 3. & ~(page_size - 1): 使用位运算将地址的低位清零，从而对齐到 page_size 的整数倍
    //    (例如，如果 page_size 是 4096 (0x1000)，那么 page_size - 1 是 4095 (0xFFF)，
    //    ~ (page_size - 1) 是 ~0xFFF，这将清除地址的低 12 位，使其对齐到 4096 的倍数)
...
void align_free(void* ptr) {
    ...
    // 我们在 align_alloc 中将 original_ptr 存储在 aligned_ptr - sizeof(void*) 的位置
...
int main(int argc, char *argv[]) {
    ...
    // 2. 获取缓冲区大小（仍然设置为一个内存页的大小）
    buffer_size = io_blocksize();
    fprintf(stderr, "使用页对齐缓冲区大小: %zu 字节\n", buffer_size);

    // 3. 使用 align_alloc 动态分配页对齐的缓冲区内存
